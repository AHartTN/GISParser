namespace GISParser.Helpers
{
	...
	public class ESRIHelper
	{
		...
		//		Console.Write($"\r{DateTime.Now} | Processing all files in {directory.Name}\t\t\t\t\t");
		...
		//		{
		//			Console.Write($"\rSkipping {fileGroup.Key} as it contained no valid files. (How did this happen?)\t\t\t\t\t");
		//		}
		...
		//			Console.Write($"\rParsing of {fileGroup.Key} is complete\t\t\t\t\t");
		//		}
		//	}
		//	catch (Exception e)
		//	{
		...
		public void ProcessFileGroup(IGrouping<string, FileInfo> fileGroup)
		{
			try
			{
				Console.Write($"\rNow handling {fileGroup.Key} files\t\t\t\t\t");
				...
				Console.Write($"\r{DateTime.Now} | {fileGroup.Key} has finished parsing\t\t\t\t\t");
			}
			catch (Exception e)
			{
				Console.WriteLine($"\r\n{e.Message}\r\n{e}");
